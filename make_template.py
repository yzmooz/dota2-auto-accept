"""
Создание эталона кнопки «ПРИНЯТЬ» из скриншота.

Нужно, если у тебя другое разрешение/язык интерфейса и стандартный эталон
images/accept_button.png не подходит.

Как пользоваться:
  1. Сделай скриншот всего экрана в момент, когда видна кнопка принятия матча
     (Win+PrintScreen сохраняет в Изображения/Снимки экрана).
  2. Запусти:
         python make_template.py путь_к_скриншоту.png
     Скрипт сам найдёт зелёную кнопку и сохранит images/accept_button.png.

Если автопоиск не сработал (кнопка не зелёная / другой цвет), можно задать
область вручную:  python make_template.py скрин.png  x y w h
"""
